  ...
  try {
    ...
    if (!user) {
      ...
      try {
        ...
        return res
          .status(400)
          .json({ success: false, message: "Failed to sync user from Clerk" });
      }
    }
...
export const purchaseCourse = async (req, res) => {
  try {
    ...
    res.json({ success: true, url: session.url, session_url: session.url });
  } catch (error) {
...
export const updateUserCourseProgress = async (req, res) => {
  try {
    ...
    if (progressData) {
      // If lecture is already marked as completed, return
